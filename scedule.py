...
def save_to_excel(schedule, month, year):
    ...
    ws = wb.active
    ws.append(["Дата", "День тижня", "Перша зміна", "Друга зміна", "Третя зміна"])
    
    for day, shifts in schedule.items():
        current_date = datetime(year, month, day)
        weekday = WEEKDAYS[current_date.weekday()]
        row = [f"{day:02}.{month:02}.{year}", weekday, shifts["Третя зміна"], shifts["Друга зміна"], shifts["Перша зміна"]]
        ws.append(row)
